...
串行跑3个工具的时间 vs 并行跑3个工具的时间
"并行架构将响应时间从约97s降至约70s"（llm大模型与agent之间的多次交流的正常响应时间较长）

> 唯一失败项：高质量PRD评分偏低（45 vs 期望 55+），Agent 对完整 PRD 存在过度扣分的倾向，待优化评分 prompt。
> 已知问题（正在修复）：对高质量PRD，Agent 的 check_completeness 工具
> 评分偏低（45分，期望55+），且缺失字段识别数过多（7个）。
> 根因：RAG 召回的规范文本以「必须包含」为主，缺少「加分项」和「评分上界」描述。
> 修复方向：在规范库中增加正向示例（高质量PRD的典型特征），
> 让 LLM 在评分时有「上界参照」。
